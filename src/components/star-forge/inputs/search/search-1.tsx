...
import { useDebounce } from '@/hooks/star-forge/use-debounce';
import { Search, X, Clock, Loader2, TrendingUp } from 'lucide-react';

import { cn } from '@/lib/utils';

export interface Search1InputOption {
  id: string;
  title: string;
  description?: string;
  category?: string;
...
export interface Search1InputProps {
  onSearch?: (query: string) => Promise<Search1InputOption[]>;
  trendingSearches?: Search1InputOption[];
  onSelect?: (item: Search1InputOption) => void;
  placeholder?: string;
  className?: string;
...
function getSearchDisplayValue(item: Search1InputOption) {
...
function SearchItem({ item }: { item: Search1InputOption }) {
  return (
...
export function Search1Input({
  ...
  const [results, setResults] = useState<Search1InputOption[]>([]);
